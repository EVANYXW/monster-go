...
var clientBufferPacker *ClientBufferPacker
...
func NewClientPacker() *ClientBufferPacker {
	clientOnce.Do(func() {
		clientBufferPacker = &ClientBufferPacker{
			//lenMsgLen: 4,
			lenMsgLen: 2,
			minMsgLen: 2,
	...
	return clientBufferPacker
}

func (p *ClientBufferPacker) GetType() string {
	return "client"
}
...
//func (p *ClientBufferPacker) PackData(msgID uint64, data []byte) ([]byte, error) {
//	//pack := p.TestPack(msgID, msg)
//	//return pack, nil
//
//	pbMsg, ok := msg.(proto.Message)
...
func (p *ClientBufferPacker) TestPack(msgId uint64, msg interface{}) []byte {
	pbMsg, ok := msg.(proto.Message)
	if !ok {
		//return []byte{}, fmt.Errorf("msg is not protobuf message")
	}
...
// conn *NetPoint
func (p *ClientBufferPacker) TestRead(c []byte) (*Message, error) {
