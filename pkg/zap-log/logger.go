...
func Game(userId, tag, msg string, params ...interface{}) {
	...
	logger.Info(fmt.Sprintf(msg, params...), zap.String("tag", GAMETAG), zap.String("userId", userId))
	//}
}

func DB(traceId, dbName, nodId, script string, count int32) {
	//if CheckLogTag(DBTAG) {
	logger.Debug(script, zap.String("tag", DBTAG), zap.String("nodeId", nodId), zap.String("db", dbName), zap.Int32("count", count), zap.String("traceId", traceId))
	//}
}
