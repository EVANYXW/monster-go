...
func printGC(memStats *runtime.MemStats, gcstats *debug.GCStats) string {
	if gcstats.NumGC > 0 {
		...
		return fmt.Sprintf("NumGC:%d Pause:%s Pause(Avg):%s Overhead:%3.2f%% Alloc:%s Sys:%s Alloc(Rate):%s/s Histogram:%s %s %s \n",
			gcstats.NumGC,
			TimeSprintf(lastPause),
			TimeSprintf(avg(gcstats.Pause)),
			overhead,
			toH(memStats.Alloc),
			toH(memStats.Sys),
			toH(uint64(allocatedRate)),
			TimeSprintf(gcstats.PauseQuantiles[94]),
			TimeSprintf(gcstats.PauseQuantiles[98]),
			TimeSprintf(gcstats.PauseQuantiles[99]))
	} else {
